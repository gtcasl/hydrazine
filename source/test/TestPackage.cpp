...
namespace test
{
	...
	////////////////////////////////////////////////////////////////////////////
	// TestPackage
	bool TestPackage::_testCopy( std::string& status )
	{
		...
		for( unsigned int i = 0; i < adds; ++i )
		{
			...
			if( !( result[ i ] == packables[ i ] )  )
			{
				...
				stream << "Object " << i << " with value " << result[i].sum() 
					<< " did not match reference " << packables[i].sum() 
					<< " .\n";
				stream << "Test Point 1 (AddRemove): Failed.\n";
				status += stream.str(); 
				return false;
	...
	bool TestPackage::_testComplex( std::string& status )
	{
					...
					if( _readCount < reads )
					{
						...
						if( pi != set.end() )
						{
							...
							if( !( packable == packables[ object ] ) )
							{
								...
								std::stringstream stream;
								stream << "Object " << object << " with value " 
								<< packable.sum() 
								<< " did not match reference " 
								<< packables[object].sum() 
								<< " .\n";
								stream << "Test Point 2 (Complex): Failed.\n";
								status += stream.str(); 
								return false;
	...
	bool TestPackage::doTest( std::string& status )
	{
...
int main( int argc, char** argv )
{
	...
	parser.parse( "-size", test.averageSize, 1024, 
